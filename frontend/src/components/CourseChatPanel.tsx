...
export const CourseChatPanel: React.FC<CourseChatPanelProps> = ({ courseSlug, courseTitle }) => {
  ...
  useEffect(() => {
    ...
    // We keep it simple: user explicitly joins the discussion first
  }, [courseSlug]);
  ...
  return (
    <section className="space-y-3">
      <div className="card-elevated p-4 md:p-5 space-y-3">
        ...
        {!hasJoined ? (
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-xs space-y-2 dark:border-slate-800 dark:bg-slate-950/60">
            <p className="font-medium text-slate-800 dark:text-slate-200">Request to join discussion</p>
            <p className="text-slate-600 dark:text-slate-400">
              Send a request to join this course chat. Once accepted, you can see and send
              messages with your peers.
            </p>
            <button
              type="button"
              onClick={() => {
