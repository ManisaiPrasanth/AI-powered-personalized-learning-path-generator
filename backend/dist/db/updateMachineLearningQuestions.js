...
function assertTen(unitNumber, questions) {
...
async function updateMachineLearningQuestions() {
    ...
    const mlCourse = await (0, connection_1.get)('SELECT id FROM Courses WHERE slug = ?', [
        'machine-learning'
    ]);
    if (!mlCourse) {
    ...
    const baseUnits = await (0, connection_1.all)('SELECT id, unit_number, title FROM Units WHERE course_id = ? AND is_detailed_version_of IS NULL ORDER BY unit_number', [mlCourse.id]);
        ...
        1: [
            {
                ...
                correct: 'b',
                explanation: 'ML learns patterns from data rather than relying only on hand-written rules.'
            },
            {
    ...
    for (const unit of baseUnits) {
        ...
        if (!unitQuestions) {
            ...
            console.warn(`Skipping Unit ${unit.unit_number} (${unit.title}): no question bank defined.`);
            continue;
        }
        assertTen(unit.unit_number, unitQuestions);
        const quiz = await (0, connection_1.get)('SELECT id FROM Quizzes WHERE unit_id = ?', [unit.id]);
        if (!quiz) {
            // eslint-disable-next-line no-console
        ...
        for (const q of unitQuestions) {
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [quiz.id, q.text, q.a, q.b, q.c, q.d, q.correct, q.explanation]);
        }
    }
