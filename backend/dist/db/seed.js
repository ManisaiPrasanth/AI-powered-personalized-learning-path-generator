...
async function seed() {
    ...
    const existingCourses = await (0, connection_1.all)('SELECT id FROM Courses LIMIT 1');
    if (existingCourses.length > 0) {
        console.log('Seed skipped: data already exists.');
        return;
    }
    // Courses
    await (0, connection_1.run)(`INSERT INTO Courses (title, slug, description, total_units)
     VALUES
    ...
    `);
    const courses = await (0, connection_1.all)('SELECT id, slug FROM Courses');
    ...
    const baseUnits = await (0, connection_1.all)('SELECT id, unit_number FROM Units WHERE course_id = ? AND is_detailed_version_of IS NULL ORDER BY unit_number', [mlCourse.id]);
    // Detailed versions (one-to-one for each unit)
    for (const unit of baseUnits) {
        await (0, connection_1.run)(`INSERT INTO Units (course_id, unit_number, title, is_detailed_version_of, detailed_content)
       VALUES (?, ?, ?, ?, ?)`, [
            mlCourse.id,
            unit.unit_number,
    ...
    const units = await (0, connection_1.all)('SELECT id, unit_number FROM Units WHERE course_id = ? AND is_detailed_version_of IS NULL ORDER BY unit_number', [mlCourse.id]);
    ...
    const quizzes = await (0, connection_1.all)(`SELECT q.id, q.unit_id, u.unit_number
     ...
     WHERE u.course_id = ? AND u.is_detailed_version_of IS NULL`, [mlCourse.id]);
