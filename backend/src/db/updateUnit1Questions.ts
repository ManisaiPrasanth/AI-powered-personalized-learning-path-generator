...
async function updateUnit1Questions() {
  ...
  const mlCourse = await get<{ id: number }>(
  ...
  const unit1 = await get<{ id: number }>(
    'SELECT id FROM Units WHERE course_id = ? AND unit_number = 1 AND is_detailed_version_of IS NULL',
    [mlCourse.id]
  );
  if (!unit1) {
  ...
  const quiz = await get<{ id: number }>(
